...
GPU::GPU() {
	...
	static const int videoController_size = 2;
	std::array<std::string, videoController_size> videoController = { "Name", "InstalledDisplayDrivers"};
...
void GPU::receiving(std::array<T, N>& v, std::string _class_name) {
		...
		if (initCom.Initialize(ObjectPath, WQL + _class_name, properties, data)) {
			...
			if (data->data_count == 0) {;
			     ...
			     a.videoCardName = dataWork.getData("Name" + std::to_string(data->data_count), data);

			     a.installedDisplayDrivers = dataWork.getData("InstalledDisplayDrivers" + std::to_string(data->data_count), data);
			...
			for (size_t i = 0; i < getAdapterCount(); ++i) {
				std::cout << "NAME " << a_buf[i].videoCardName << std::endl;
				std::cout << "InstalledDisplayDrivers " << a_buf[i].installedDisplayDrivers << std::endl;
			}
